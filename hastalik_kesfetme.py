...
# kullanıcıdan veri alalık 
print("Lütfen tahmin için aşağıdaki bilgileri giriniz:")
yas=int(input("Yaşınızı giriniz:"))
kan_basinci=float(input("Kan basıncınızı giriniz:"))
kolesterol=float(input("Kolesterol seviyenizi giriniz:"))
# Kullanıcıdan alşınan veriyi modelin anlayacağı formata çevirelim
yeni_veri=pd.DataFrame([[yas,kan_basinci,kolesterol]],columns=['Yas','Kan_Basinci','Kolesterol'])

tahmin=classifier.predict(yeni_veri)
# Tahminin ne anlama geldiğini kullanıcıya bildirin
if tahmin[0]  == 0:
    print("Hastalığınız Var!.")
else:
    print("Hastalığınız Yok!")    
